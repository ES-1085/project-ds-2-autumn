...
REPORT: There was a significant impact of the oil spill on the weight of the adult sea otter population in the Western Aleutians (Welch Two Sample t-test; t = 7.4412, df = 399.18, p-value < 0.005). An assessment of the r-squared value suggests that the variable `oil_spill` accounts for 1.96% of the variation in the dependent variable, `WEIGHT` in the western Aluetians.

### Otter's Weight Year by Year  ***get rid of this??
We will be continuing to assess the otter populations in the Western Aleutians, as this area is the only area to have otter captures both before and after the oil spill. 
...
REPORT: There was no significant impact of the oil spill on the body length of the sea otter population (Welch Two Sample t-test; t = -1.0462, df = 350.38, p-value = 0.2962). 
...
seot_west_aleutians %>% 
  ...
  filter(oil_spill == "before_oil_spill"|oil_spill == "after_oil_spill") %>%
  filter(REGION == "west_aleutians") %>% 
  ggplot(mapping = aes(y = BACULA_LGTH, x = oil_spill)) + 
  geom_boxplot() +
...
#two-way ANOVA
two_anova_lgth_oil <- aov(true_standard_lgth ~ oil_spill + FINAL_AGE + oil_spill:FINAL_AGE, data = seot_west_aleutians)
...
### Prince William Sound Otter Decline

The "Sea Otter Population Assessment" portion of the Nearshore Marine Ecosystem Research webpage of the Alaska Science Center states that, "there is good evidence that the recent declines in sea otters in SW Alaska are related to killer whale predation and the Exxon Valdez oil spill reduced the size of the western Prince William Sound population in 1989" (Alaska Science Center 2024) (website = https://www.usgs.gov/centers/alaska-science-center/science/nearshore-marine-ecosystem-research). 

We would like to analyze this data to see if we find this to be true for this dataset. 

#### Creating Plots

```{r otter-pop-pws-historical}
seot %>% 
  filter(REGION == "prince_william_sound") %>% 
  ggplot(mapping = aes(x = YEAR)) + 
  geom_histogram()
```
