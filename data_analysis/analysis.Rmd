...
# Glimpsing the Data
## Yearly Observations
First, let's look at the yearly observations for the sea otters. 

[add something here about the gap between 1970 and 1985]. 
...
seot_pws_wa %>% 
  filter(AGE_CATEGORY > -1) %>% 
  filter(FINAL_AGE > -1) %>% 
  ggplot(data = seot_pws_wa, mapping = aes(x = WEIGHT)) + 
  geom_histogram() + 
